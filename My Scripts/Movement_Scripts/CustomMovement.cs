...
namespace LowPoly.Character
{
    ...
    public class CustomMovement : MonoBehaviour
    {
        ...
        void HandleAirborneMovement(Vector3 moveInput)
        {
        ...
        void HandleGroundedMovement(bool jump)
        {
            ...
            if (jump && m_Animator.GetCurrentAnimatorStateInfo(0).IsName("Grounded"))
            {
                ...
                //GameObject jumpDust = Instantiate(m_ParticleSystem, new Vector3(transform.position.x,transform.position.y +0.85f, transform.position.z + 1.7f), Quaternion.identity, this.transform);
                //jumpDust.GetComponent<ParticleSystem>().Play(); //if we put these 3 lines in, jump will be executed 6 times for some reason.
                //Destroy(jumpDust, 2f);
                //m_ParticleSystem.GetComponentInChildren<ParticleSystem>().Play(); //this might not run if memory is low
                m_IsGrounded = false;
